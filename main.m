...
[t3, X3] = ode45(@(t,x) Chua_DINGND(t,x), tspan, x3, opts);
...
for k = 1:numel(cases)
    ...
    N = numel(Tk);
    tailN = max(5, floor(tail_frac * N));
    final_level = median(ef(max(1,N-tailN+1):N));
    thr  = (1 + eta_rel) * final_level;
    winN = max(3, floor(win_frac * N));
    ts_idx = N; 
    for i = 1:max(1, N-winN+1)
        if all(ef(i:min(i+winN-1,N)) <= thr)
            ts_idx = i; break;
        end
    end
    ...
    MSSRE = max(ef_ss);
    if numel(T_ss) >= 2
        ASSRE = trapz(T_ss, ef_ss) / max(T_ss(end)-T_ss(1), eps);
    else
        ASSRE = ef_ss(end);  
    end
